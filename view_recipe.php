...
/* echo "$recipe"; */
$sql = "SELECT * FROM recipe WHERE recipe_name = '$recipe'";
/* echo "$sql"; */
$result = $db->query($sql);
/* var_dump($result);  */
...
$sql= "SELECT ingredient_name FROM recipe_info WHERE recipe_name = '$recipe'";
$result = $db->query($sql);
echo "<ol>";
...
$sql = "SELECT tag FROM recipe_tags WHERE recipe_name = '$recipe'";
$result = $db->query($sql);
echo "<h2>Tags</h2>";
echo "<ul>";
while($row = $result->fetch_row()) {
